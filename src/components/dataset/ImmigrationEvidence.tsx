...
export function ImmigrationEvidence({ evidence }: { evidence: Evidence }) {
  ...
  return (
    <section aria-label="Chancenkarte to Blue Card evidence" className="rounded-lg border border-primary/20 bg-primary/[0.03] p-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div><h4 className="font-semibold">Chancenkarte → Blue Card</h4><p className="text-xs text-muted-foreground">Local authority evidence; card production is not treated as total processing time.</p></div>
      ...
      <div className="mt-3 border-t pt-3"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Naturalisation · context only, not scored</div><p>{evidence.naturalisation}</p></div>
    </section>
  );
}
